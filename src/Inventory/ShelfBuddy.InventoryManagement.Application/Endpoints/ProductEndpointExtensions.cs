    ...
    public static IEndpointRouteBuilder MapProductEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/{id:guid}",
            async ([FromServices] IProductRepository productRepository,
                [FromServices] IProductCategoryRepository productCategoryRepository, Guid id) =>
            {
                var product = await productRepository.GetByIdAsync(id);
                return product is null
                    ? Results.NotFound()
                    : Results.Ok(new ProductDto(product.Id, product.Name,
                        new ProductCategoryDto(product.ProductCategory.Id, product.ProductCategory.Name,
                            productCategoryRepository.GetLastUpdated(product.ProductCategory)),
                        productRepository.GetLastUpdated(product)));
            })
            .WithName("GetProduct");

        group.MapGet("/", async (HttpContext context, [FromServices] IProductRepository productRepository,
            [FromServices] IProductCategoryRepository productCategoryRepository, [FromQuery] int page = 1,
            [FromQuery] int pageSize = 10, [FromQuery] string? productCategory = null) =>
            {
                ...
                return Results.Ok(products.Select(x => new ProductDto(x.Id, x.Name,
                    new ProductCategoryDto(x.ProductCategory.Id, x.ProductCategory.Name,
                        productCategoryRepository.GetLastUpdated(x.ProductCategory)),
                    productRepository.GetLastUpdated(x))));
            })
